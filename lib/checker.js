import { fetchDeprecation } from './npm.js';
...
/** Turn a directory entry into a status. May make one extra request. */
async function classify(name, entry, request) {
  if (!entry) return { status: STATUS.UNLISTED, newArchOnly: false };

  const flag = entry.newArchitecture;
  if (flag === false) return { status: STATUS.INCOMPATIBLE, newArchOnly: false };
  if (flag === true) return { status: STATUS.COMPATIBLE, newArchOnly: false };
  if (flag === 'new-arch-only') return { status: STATUS.COMPATIBLE, newArchOnly: true };
...
export async function checkPackage(dependency, options = {}) {
  ...
  try {
    // The two registries are independent, so ask both at once. The npm lookup
    // resolves to null rather than throwing, so it can never fail the scan.
    const [entry, deprecated] = await Promise.all([
      searchWithRetry(name, request),
      checkDeprecation ? fetchDeprecation(name, { fetchImpl, timeoutMs }) : Promise.resolve(null),
    ]);

    const { status, newArchOnly } = await classify(name, entry, request);

    return {
      ...base,
      status,
      newArchOnly,
      unmaintained: Boolean(entry?.unmaintained),
      deprecated,
    };
  } catch (error) {
