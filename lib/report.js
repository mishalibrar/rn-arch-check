...
export function formatReport({ projectName, rnVersion, results, skipped = [] }) {
  ...
  if (rnVersion) lines.push(chalk.gray(`react-native ${rnVersion}`));

  const body = rows.map((row) => {
    const meta = META_BY_KEY[row.status];
    const notes = [];
    if (row.newArchOnly) notes.push('New Arch only');
    if (row.unmaintained) notes.push('unmaintained');
    if (row.error) notes.push(row.error);
    const suffix = notes.length ? chalk.gray(`  (${notes.join(', ')})`) : '';
